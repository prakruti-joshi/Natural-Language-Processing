...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, num_word_types, num_tag_types, num_char_types, dim_word,
        ...
        if not nochar:
            ...
            self.c = BiLSTMOverCharacters(self.cemb, num_layers)

        dim_input = dim_word if nochar else dim_word + 2 * dim_char
        print("Word dim", dim_word)
        print("Char dim", dim_char)
        print("Input dim",dim_input)
    ...
    def score(self, X, Y, C, C_lengths):
        ...
        scores = self.drop(self.scorer(output))  # B x T x L    L = num_tag_types
        return scores
