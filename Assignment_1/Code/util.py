...
class BigramLanguageModel:
    ...
    def compute_bigram_prob_interpolation(self, beta, alpha):
        ...
        self.compute_bigram_prob_laplace(alpha)   # Laplace smoothing

        unigram_counts = np.sum(self.bi_counts, axis=0)
        corpus_size = np.sum(self.bi_counts)

        for i, _ in enumerate(self.bi_prob):
            self.bi_prob[i] = beta*(self.bi_prob[i]) + (1-beta)*(unigram_counts+alpha)/(corpus_size+alpha*self.n)
